...
//------------------------------------------------------------------------------
TMdMessage::TMdMessage( const MTree& Ref )
:FTree( Ref )
{
	Attach( &FTree );
}
...
//------------------------------------------------------------------------------
void TMdMessage::SetZIPStringValue( const char* FieldName, const UFC::AnsiString& Value )
{
    if( Value.Length() > 0 )
    {
        MNode* ExistNode	= FTreePtr->get( FieldName );
        MNode* NewNode		= new MNode( FieldName, (const unsigned char*)Value.c_str(), Value.Length() + 1, TRUE );
        SetValue( ExistNode, NewNode );
    }
}
//------------------------------------------------------------------------------
