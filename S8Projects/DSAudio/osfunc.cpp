...
{
   unsigned int i = (unsigned int)path.rfind('\\');
   unsigned int j = (unsigned int)path.rfind('/');
...
std::string relative_path(const std::string &to_path, const std::string &from_path)
{
    ...
    // correct the '\' char
	pos = 0;
	while((pos= (unsigned int)p1.find("\\", pos))!= p1.npos) p1.replace(pos, 1, "/");
	pos = 0;
	while((pos= (unsigned int)p2.find("\\", pos))!= p2.npos) p2.replace(pos, 1, "/");
	...
	{
		pos = i;
		while((pos=(unsigned int)p2.find("/", pos))!= p2.npos)
		{
			output += "../";
			pos ++;
		}
	...
	if(i< p1.size())
	{
		while((pos=(unsigned int)p1.find("/", i))!= p1.npos)
		{
			int count = pos - i;
