...
{
    UFC::BufferedLog::DebugPrintf( " Reject order:[%s].", ClOrdID.getString().c_str() );
...
void OrcTSEMapper::onMessage( const FIX44::NewOrderSingle& Order, const FIX::SessionID& sessionID )
{
    ...
    {
        Reject( OIDStr.c_str(), GetErrorMessage( "31014" ), Side, ClOrdID, Symbol, OrderType, OrderQty ) ;
        return;
    }
    
    
    ///< T32 check
    if( T32Check( Symbol.getValue().c_str() ) == FALSE )
    {
        Reject( OIDStr.c_str(), "Can not trade foreign ETF", Side, ClOrdID, Symbol, OrderType, OrderQty ); ///<Cannot trade in the odd lots of this stock.        
        return;
    }
    ...
    if( ( Qty % 1000 ) > 0)
    {
        Reject( OIDStr.c_str(), GetErrorMessage("ODD0022").c_str(), Side, ClOrdID, Symbol, OrderType, OrderQty ); ///<Cannot trade in the odd lots of this stock.
        return;
    }
    
    if( Qty >= 499000 )
    {
        Reject( OIDStr.c_str(), GetErrorMessage("ODD0014").c_str(), Side, ClOrdID, Symbol, OrderType, OrderQty ); ///<Odd lot orders cannot exceed 499 shares.
        return;
    }
    ...
    ClientOrderID NID = ClientOrderID::CreateClientOrderID( mkTSE, tkNewOrder);
    ...
    if( Side == FIX::Side_SELL ) ///< 2 for sell
    {
        ...
        if( LStore->CheckLeftQty( TRUE , ChkAccount, SymbolStr, Qty, SaleID, Reason ) == FALSE )
        {
    ...
    TSCTSEOrderRecord* NewOrder = new TSCTSEOrderRecord( NID, OIDStr, ClOrdID, Account, Symbol, OrderQty, Price, Side, timeInForce, SaleID, FMsgSource, IsMarket );
    FStore->AddOrder( (Int32)NID, NewOrder );
    NewOrder->NewOrderString( R010 );
    UFC::BufferedLog::DebugPrintf( " Stock Order:[%s]", R010.c_str() );
