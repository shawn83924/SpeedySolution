...
//---------------------------------------------------------------------------
const UFC::AnsiString AccountFile( "../cfg/account.txt");
const UFC::AnsiString LeftQtyFile( "../cfg/prelock.csv" );
//---------------------------------------------------------------------------
...
BOOL TCheckLeftQty::Check( ICheckLeftQtyParam& Param,  LeftQtyStore* LStore )
{
    ...
    if( FRequestString.Length() > 0 )
    {
        try
        {
            UFC::BufferedLog::DebugPrintf(" Request string:[%s]", FRequestString.c_str() );
            FSocket->Connect();
            FResult = 0x00;
...
void LeftQtyStore::ReadTSCDConfig( void )
{
    ...
    UFC::UiniFile     Config( "../cfg/GLTSEMapper.cfg" );
