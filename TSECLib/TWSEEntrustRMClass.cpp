...
//------------------------------------------------------------------------------
CEntrustRiskManagerClass::CEntrustRiskManagerClass(bool IsRecoverMode)
:CTWSERiskManagerClass(IsRecoverMode)
,FWhiteListFileSecond(0LL)
{
    UFC::BufferedLog::Printf(" CEntrustRiskManagerClass::%s()", __func__);
}  //CEntrustRiskManagerClass::CEntrustRiskManagerClass()
//------------------------------------------------------------------------------
CEntrustRiskManagerClass::~CEntrustRiskManagerClass()
{
}  //CEntrustRiskManagerClass::~CEntrustRiskManagerClass()
//------------------------------------------------------------------------------
void CEntrustRiskManagerClass::Check(CheckData& Obj)
{
}  //CEntrustRiskManagerClass::Check()
//---------------------------------------------------------------------------------------------------------------------
void CEntrustRiskManagerClass::ParseRMDataFromUserData(const UFC::AnsiString& Key, UFC::AnsiString& WarrantNo, UFC::AnsiString& BrokerID)
{
    WarrantNo = "";
    BrokerID = "";
...
}  //CEntrustRiskManagerClass::ParseRMDataFromUserData()
//------------------------------------------------------------------------------
void CEntrustRiskManagerClass::RecoverRMConfirmData(const UFC::AnsiString& ExecRptDataStr, const UFC::AnsiString& WarrantNo, const UFC::AnsiString& BrokerID, Int32 NetworkID, bool IsSynchronize)
{
    ...
    CRMOrderSummary *targetOrderPtr = gEntrustRMDataPoolPtr->FindOrderFromIDIndex(orderID, UFC::rwLockForRead);
    if ((functionCode == 1) || (functionCode == 2))
    {
        if (targetOrderPtr == 0)
        {
            ...
            if (gEntrustRMDataPoolPtr->InsertOrderToIDIndex(orderID, newOrderPtr, UFC::cstCheck, UFC::rwLockForWrite))
    ...
    if (targetOrderPtr != 0)
    {
        CRMEntrustInvestorAccount *accountPtr = gEntrustRMDataPoolPtr->FindAccountFromNoIndex(accountNo, UFC::rwLockForRead);
        CRMInvestorSubAccount* subAccountPtr = gEntrustRMDataPoolPtr->FindSubAccountFromNoIndex(accountNo, WarrantNo, UFC::rwLockForRead);
        if (accountPtr != 0)
        {
            bool isNewStockData = false;
            CRMEntrustStockTradingData *stockDataPtr = accountPtr->FindOrCreateStockTradingData(stockNo, isNewStockData, UFC::rwLockForWrite);
            if (stockDataPtr != 0)
            {
                if (isNewStockData)
                ...
                if ((functionCode == 1) || (functionCode == 2))
                {
                    ...
                    if (isInsertOrderSuccess)
                    {
                        if (subAccountPtr != 0)
                        {
                            subAccountPtr->LockForWrite();
                            if (functionCode == 1)
                            {
                                double origBuyOrderAmount = subAccountPtr->GetBuyOrderAmount(UFC::rwNotLock);
                                double newBuyOrderAmount = origBuyOrderAmount + orderAmount;
                                subAccountPtr->SetBuyOrderAmount(newBuyOrderAmount, UFC::rwNotLock);
                                FLogPtr->fprintf(" %s Set SubAccount[%s][%s] BuyOrderAmount from [%15.4lf] to [%15.4lf].", logHead.c_str(), accountNo.c_str(), WarrantNo.c_str(), origBuyOrderAmount, newBuyOrderAmount);
                            }
                            else
                            {
                                double origSellOrderAmount = subAccountPtr->GetSellOrderAmount(UFC::rwNotLock);
                                double newSellOrderAmount = origSellOrderAmount + orderAmount;
                                subAccountPtr->SetSellOrderAmount(newSellOrderAmount, UFC::rwNotLock);
                                FLogPtr->fprintf(" %s Set SubAccount[%s][%s] SellOrderAmount from [%15.4lf] to [%15.4lf].", logHead.c_str(), accountNo.c_str(), WarrantNo.c_str(), origSellOrderAmount, newSellOrderAmount);
                            }
                            FLogPtr->fprintf(" %s %s.", logHead.c_str(), subAccountPtr->ToString(UFC::rwNotLock).c_str());
                            subAccountPtr->UnLockForWrite();
                        }  //if (subAccountPtr != 0)
                ...
                else if ((functionCode == 3) || (functionCode == 4))
                {
                    ...
                    targetOrderPtr->SetLeavesQty(afterQty, UFC::rwNotLock);
                    ...
                    if (functionCode == 3)
                    {
                        ...
                        FLogPtr->fprintf(" %s Reduce Order[%s] Qty from [%d] to [%d], LeavesQty from [%d] to [%d], Amount from [%15.4lf] to [%15.4lf].", logHead.c_str(),
                                         orderID.c_str(), origOrderQty, newOrderQty, origLeavesQty, afterQty, origOrderAmount, newOrderAmount);
                    }  //if (functionCode == 3)
                    ...
                    if (subAccountPtr != 0)
                    {
                        ...
                        FLogPtr->fprintf(" %s Set Order[%s] LeavesQty from %d to %d, Amount from %15.4lf to %15.4lf.", logHead.c_str(),
                                         orderID.c_str(), origLeavesQty, afterQty, origOrderAmount, newOrderAmount);
                    }  //if (subAccountPtr != 0)
                }
                else if (functionCode == 6)
                {
                ...
                char conditionOrderType = '0';
                if ((twseOrderType == '5') || (twseOrderType == '6')) conditionOrderType = '5';
                CRMEntrustStockCondition *conditionPtr = stockDataPtr->FindCondition(WarrantNo, conditionOrderType, UFC::rwLockForRead);
                bool isSellShort = BrokerID != CEntrustRiskManagerDataPool::GetRMMainBrokerID();  //是否為借券
                if ((conditionPtr == 0) && !isSellShort)  //非借券
                {
                    UFC::AnsiString todayStr;
                    UFC::GetYYYYMMDD(todayStr, FALSE);
                    conditionPtr = new CRMEntrustStockCondition(WarrantNo, conditionOrderType, 0, 0, isSellShort);
                    conditionPtr->SetAccountNo(accountNo, UFC::rwNotLock);
                    conditionPtr->SetStockNo(stockNo, UFC::rwNotLock);
                    if (!stockDataPtr->InsertCondition(WarrantNo, conditionOrderType, conditionPtr, UFC::cstNotCheck, UFC::rwLockForWrite))
                    {
...
}  //CEntrustRiskManagerClass::RecoverRMConfirmData()
//------------------------------------------------------------------------------
void CEntrustRiskManagerClass::RecoverRMFillData(const UFC::AnsiString& ExecRptDataStr, const UFC::AnsiString& WarrantNo, const UFC::AnsiString& BrokerID, bool IsSynchronize)
{
    UFC::AnsiString logHead = "";
    if (IsSynchronize)
...
//    execKey.Printf( "%s_%08d_%08d%c", BrokerID.c_str(), matchTime, matchSeqNo, side );
    ...
    CRMOrderSummary *orderPtr = gEntrustRMDataPoolPtr->FindOrderFromIDIndex(orderID, UFC::rwLockForRead);
    if (orderPtr != 0)
    {
        ...
        if (origLeavesOrderQty > 0)
        {
            if (origLeavesOrderQty >= matchQty)
            {
                ...
                if (accountPtr != 0)
                {
                    ...
                    CRMEntrustStockTradingData *stockDataPtr = accountPtr->FindStockTradingData(stockNo, UFC::rwNotLock);
                    if (stockDataPtr != 0)
                    {
                        ...
                        char conditionOrderType = '0';
                        if ((twseOrderType == '5') || (twseOrderType == '6')) conditionOrderType = '5';
                        CRMEntrustStockCondition *conditionPtr = stockDataPtr->FindCondition(orderWarrantNo, conditionOrderType, UFC::rwNotLock);
                    ...
                    if (subAccountPtr != 0)
                    {
                        ...
                        if (buySellCode == 'B')
                        {
                            double origBuyMatchAmount = subAccountPtr->GetBuyMatchAmount(UFC::rwNotLock);
                            double newBuyMatchAmount = origBuyMatchAmount + matchAmount;
                            subAccountPtr->SetBuyMatchAmount(newBuyMatchAmount, UFC::rwNotLock);
                            ...
                            subAccountPtr->SetBuyOrderAmount(newBuyOrderAmount, UFC::rwNotLock);
                            FLogPtr->fprintf(" %s Set SubAccount[%s][%s] BuyMatchAmount from [%15.4lf] to [%15.4lf], BuyOrderAmount from [%15.4lf] to [%15.4lf].", logHead.c_str(),
                                             accountNo.c_str(), WarrantNo.c_str(), origBuyMatchAmount, newBuyMatchAmount, origBuyOrderAmount, newBuyOrderAmount);                        
                        }
                        else
                        {
                            double origSellMatchAmount = subAccountPtr->GetSellMatchAmount(UFC::rwNotLock);
                            double newSellMatchAmount = origSellMatchAmount + matchAmount;
                            subAccountPtr->SetSellMatchAmount(newSellMatchAmount, UFC::rwNotLock);
                            FLogPtr->fprintf(" %s Set SubAccount[%s][%s] SellMatchAmount from [%15.4lf] to [%15.4lf].", logHead.c_str(), accountNo.c_str(), WarrantNo.c_str(), origSellMatchAmount, newSellMatchAmount);
                            ...
                            subAccountPtr->SetSellOrderAmount(newSellOrderAmount, UFC::rwNotLock);
                            FLogPtr->fprintf(" %s Set SubAccount[%s][%s] SellMatchAmount from [%15.4lf] to [%15.4lf], SellOrderAmount from [%15.4lf] to [%15.4lf].", logHead.c_str(),
                                             accountNo.c_str(), WarrantNo.c_str(), origSellMatchAmount, newSellMatchAmount, origSellOrderAmount, newSellOrderAmount);                        
                        }  //if (buySellCode == 'B')
                        subAccountPtr->UnLockForWrite();
                    }  //if (subAccountPtr != 0)
        ...
        FLogPtr->fprintf(" %s Order[%s] not Found.", logHead.c_str(), orderID.c_str());
}  //CEntrustRiskManagerClass::RecoverRMFillData()
//------------------------------------------------------------------------------
void CEntrustRiskManagerClass::ParseExecRtpLogStr(const UFC::AnsiString& ExecRptLogStr, bool IsSynchronize)
{
    UFC::AnsiString logHead = "CEntrustRiskManagerClass::ParseExecRtpLogStr()";
...
//        WriteRiskLog(logHead, "Parse Execution[%s]", ExecRptLogStr.c_str());
    ...
    FLogPtr->Flush();
}  //CEntrustRiskManagerClass::ParseExecRtpLogStr()
//------------------------------------------------------------------------------
void CEntrustRiskManagerClass::RecoverRMData()
{
    UFC::AnsiString logHead = "CEntrustRiskManagerClass::RecoverRMData()";
...
}  //CEntrustRiskManagerClass::RecoverRMData()
//---------------------------------------------------------------------------------------------------------------------
bool CEntrustRiskManagerClass::CheckBuyOrder(CRMOrderSummary *OrderPtr, CRMEntrustInvestorAccount *AccountPtr, CRMInvestorSubAccount* SubAccountPtr, CRMEntrustStockTradingData *StockDataPtr, UFC::AnsiString& CheckMessage)
{
    UFC::AnsiString logHead = "CEntrustRiskManagerClass::CheckBuyOrder()";
    bool isPassed = true;
    ...
    if ((msgType != TWSE::mtO010) && (msgType != TWSE::mtO110) && (twseOrderType != '0'))  //零股沒有借券
        isSellShort = true;
    
    char conditionOrderType = '0';
    if ((twseOrderType == '5') || (twseOrderType == '6')) conditionOrderType = '5';
    CRMEntrustStockCondition *conditionPtr = StockDataPtr->FindCondition(warrantNo, conditionOrderType, UFC::rwNotLock);
    if (conditionPtr == 0)
    {
        if (!isSellShort)
        {
            FLogPtr->fprintf(" %s Buy Condition Account[%s] Stock[%s] SubAccount[%s] BrokerID[%s] TWSEOrdType[%c] not Found.",
                             logHead.c_str(), accountNo.c_str(), stockNo.c_str(), warrantNo.c_str(), brokerID.c_str(), twseOrderType);
            UFC::AnsiString todayStr;
            UFC::GetYYYYMMDD(todayStr, FALSE);
            conditionPtr = new CRMEntrustStockCondition(warrantNo, conditionOrderType, availableSellQty, availableBuyQty, isSellShort);
            conditionPtr->SetAccountNo(accountNo, UFC::rwNotLock);
            conditionPtr->SetStockNo(stockNo, UFC::rwNotLock);
            if (!StockDataPtr->InsertCondition(warrantNo, conditionOrderType, conditionPtr, UFC::cstNotCheck, UFC::rwNotLock))
            {
                CheckMessage.Printf("Reject[%s]: Insert Account[%s] Stock[%s][%s]%c condition failed", orderID.c_str(), accountNo.c_str(), stockNo.c_str(), warrantNo.c_str(), twseOrderType);
                delete conditionPtr;
                conditionPtr = 0;
            }
        }
        else
            CheckMessage.Printf("Reject[%s]: Account[%s] Stock[%s][%s]%c not found", orderID.c_str(), accountNo.c_str(), stockNo.c_str(), warrantNo.c_str(), twseOrderType);
...
//                     logHead.c_str(), availableSellQty, normalMatchSellQty, normalMatchBuyQty, matchBuyQty, normalOrderSellQty, orderSellQty);
    ...
    return isPassed;
}  //CEntrustRiskManagerClass::CheckBuyOrder()
//---------------------------------------------------------------------------------------------------------------------
bool CEntrustRiskManagerClass::CheckSellOrder(CRMOrderSummary *OrderPtr, CRMEntrustInvestorAccount *AccountPtr, CRMInvestorSubAccount* SubAccountPtr, CRMEntrustStockTradingData *StockDataPtr, UFC::AnsiString& CheckMessage)
{
    UFC::AnsiString logHead = "CEntrustRiskManagerClass::CheckSellOrder()";
    bool isPassed = true;
    OrderPtr->LockForRead();
    TWSE::TMessageType msgType = OrderPtr->GetMsgType(UFC::rwNotLock);
    UFC::AnsiString orderID = OrderPtr->GetOrderID(UFC::rwNotLock);
    ...
    if (conditionPtr == 0)
    {
        if (!isSellShort)
        {
            FLogPtr->fprintf(" %s Sell Condition Account[%s] Stock[%s] SubAccount[%s] TWSEOrdType[%c] not Found.",
                             logHead.c_str(), accountNo.c_str(), stockNo.c_str(), warrantNo.c_str(), twseOrderType);
            UFC::AnsiString todayStr;
            UFC::GetYYYYMMDD(todayStr, FALSE);
            conditionPtr = new CRMEntrustStockCondition(warrantNo, conditionOrderType, availableSellQty, availableBuyQty, isSellShort);
            conditionPtr->SetAccountNo(accountNo, UFC::rwNotLock);
...
//    int intraDayOddOrderBuyQty = conditionPtr->GetIntraDayOddOrderBuyQty(UFC::rwNotLock);    //今日日盤零股委託買進股數
    ...
    UFC::PStringBuffer availableBuffer;
    if (!isSellShort && ((twseOrderType == '5') || (twseOrderType == '6')))
    {
    ...
    else
    {
        if (AccountPtr->NeedCheckSellPosition(UFC::rwNotLock))  //Check Sell Position
        {
            ...
            if ((msgType != TWSE::mtT010) && (msgType != TWSE::mtO110))
            {
            ...
            if (dayTradeType == 1) 
            {
                if (CEntrustRiskManagerDataPool::IsRMHedgeStock(stockNo, AccountPtr->GetAccountNo(UFC::rwNotLock)))
            ...
            if (!isSellShort && (dayTradeType == 1) &&  //Can Sell Buy Today Qty
                (msgType != TWSE::mtO110) && (msgType != TWSE::mtO010))  //not Odd
            {
...
//                }
    ...
    return isPassed;
}  //CEntrustRiskManagerClass::CheckSellOrder()
//---------------------------------------------------------------------------------------------------------------------
bool CEntrustRiskManagerClass::CheckReplacePriceOrder(CRMOrderSummary *OrderPtr, CRMEntrustInvestorAccount *AccountPtr, CRMEntrustStockTradingData *StockDataPtr, CRMOrderSummary *OrderRecPtr, UFC::AnsiString& CheckMessage)
{
    UFC::AnsiString logHead = "CEntrustRiskManagerClass::CheckReplacePriceOrder()";
    bool isPassed = true;
    OrderPtr->LockForRead();
    TWSE::TMessageType msgType = OrderPtr->GetMsgType(UFC::rwNotLock);
    ...
    if (orderPrice > origOrderPrice)
    {
        ...
        else
        {
            ...
            isPassed = subAccountPtr->CheckOrderAmount(orderID, buySellCode, increaseAmount, CheckMessage, UFC::rwLockForRead);
        }
    }
    ...
    if (isPassed)
    {
        ...
        if (buySellCode == 'B')
        {
            ...
            if (subAccountPtr != 0)
            {
                ...
                subAccountPtr->SetBuyOrderAmount(newBuyOrderAmount, UFC::rwNotLock);
                subAccountPtr->UnLockForWrite();
                FLogPtr->fprintf(" %s Set SubAccount[%s][%s] Buy OrderAmount from [%15.4lf] to [%15.4lf].", logHead.c_str(), accountNo.c_str(), warrantNo.c_str(), origBuyOrderAmount, newBuyOrderAmount);
            }
        }
        else
        {
            ...
            if (subAccountPtr != 0)
            {
                ...
                subAccountPtr->SetSellOrderAmount(newSellOrderAmount, UFC::rwNotLock);
                subAccountPtr->UnLockForWrite();
                FLogPtr->fprintf(" %s Set Account[%s][%s] Sell OrderAmount from [%15.4lf] to [%15.4lf].", logHead.c_str(), accountNo.c_str(), warrantNo.c_str(), origSellOrderAmount, newSellOrderAmount);
            }
        }
        FLogPtr->fprintf(" %s %s.", logHead.c_str(), AccountPtr->ToString(UFC::rwNotLock).c_str());
        FLogPtr->fprintf(" %s Stock: %s, Order[%s], MsgType[%d].", logHead.c_str(), StockDataPtr->ToString(UFC::rwNotLock).c_str(), orderID.c_str(), msgType);
    ...
    return isPassed;
}  //CEntrustRiskManagerClass::CheckReplacePriceOrder()
//---------------------------------------------------------------------------------------------------------------------
bool CEntrustRiskManagerClass::CheckRiskCondition(TWSE::MessageHeader*& Msg, OrderRecord& OI, UFC::AnsiString& CheckMessage)
{
    ...
    UFC::AnsiString logHead = "CEntrustRiskManagerClass::CheckRiskCondition()";
    FLogPtr->fprintf(" %s", logHead.c_str());
    ...
    UFC::AnsiString warrantNo = orderPtr->GetWarrantNo(UFC::rwNotLock);  //SubAccount
    UFC::AnsiString brokerID = orderPtr->GetBrokerID(UFC::rwNotLock);
    char buySellCode = orderPtr->GetSide(UFC::rwNotLock);
    char twseOrderType = orderPtr->GetTWSEOrderType(UFC::rwNotLock);
    int orderQty = orderPtr->GetOrderQty(UFC::rwNotLock);
    ...
    CRMOrderSummary *existOrderPtr = gEntrustRMDataPoolPtr->FindOrderFromIDIndex(orderID, UFC::rwLockForRead);
    if ((functionCode == 1) || (functionCode == 2))  //buy or sell
    {
        ...
        {
            if (gEntrustRMDataPoolPtr->InsertOrderToIDIndex(orderID, orderPtr, UFC::cstNotCheck, UFC::rwLockForWrite))
    ...
    if (accountPtr != 0)
    {
        ...
        if ((twsePriceType == '1') &&  //Market
            CEntrustRiskManagerDataPool::NeedCheckSelfTrading() && accountPtr->NeedCheckSelfTrade(UFC::rwNotLock))
        {
        ...
        else
        {
            if (accountPtr->NeedCheckStockWhiteList(UFC::rwNotLock) && 
                !gEntrustRMDataPoolPtr->IsRMStockExistsWhiteList(stockNo))
            {
        ...
        if (isPassed)
        {
            if (stockDataPtr != 0)
            {
                ...
                if (CEntrustRiskManagerDataPool::NeedCheckSelfTrading() && accountNeedCheckSelfTrade)
                {
                    if (buySellCode == 'B')
                    {
                ...
                {
                    if (!CEntrustRiskManagerDataPool::NeedCheckSelfTrading())
                ...
                if (isPassed)
                {
                    ...
                    {
                        CRMInvestorSubAccount* subAccountPtr = gEntrustRMDataPoolPtr->FindSubAccountFromNoIndex(accountNo, warrantNo, UFC::rwLockForRead);
                        if ((subAccountPtr == 0) ||  //Need not Check
                            subAccountPtr->CheckOrderAmount(orderID, buySellCode, orderAmount, CheckMessage, UFC::rwLockForRead))
                        {
                            if (functionCode == 1)
                                isPassed = CheckBuyOrder(orderPtr, accountPtr, subAccountPtr, stockDataPtr, CheckMessage);
                            else
                                isPassed = CheckSellOrder(orderPtr, accountPtr, subAccountPtr, stockDataPtr, CheckMessage);
                        }
                        else
                            isPassed = false;
                    }
    ...
    FLogPtr->Flush();
    return isPassed;
}  //CEntrustRiskManagerClass::CheckRiskCondition()
//---------------------------------------------------------------------------------------------------------------------
void CEntrustRiskManagerClass::ProcessRiskRejectMessage(TWSE::MessageHeader*& ReplyMsg, OrderRecord& OI)
{
    int functionCode = OI.FuncCode;
    if ((functionCode != 1) && (functionCode != 2) && (functionCode != 6)) return;

    UFC::AnsiString logHead = "CEntrustRiskManagerClass::ProcessRiskRejectMessage()";
    ...
    if (orderPtr != 0)
    {
        ...
        if (accountPtr != 0)
        {
            ...
            CRMEntrustStockTradingData *stockDataPtr = accountPtr->FindOrCreateStockTradingData(stockNo, UFC::rwNotLock);
            accountPtr->UnLockForWrite();
            ...
            if (stockDataPtr != 0)
            {
                ...
                char conditionOrderType = '0';
                if ((twseOrderType == '5') || (twseOrderType == '6')) conditionOrderType = '5';
               CRMEntrustStockCondition *conditionPtr = 0;
                if ((functionCode == 1) || (functionCode == 2))
                    conditionPtr = stockDataPtr->FindCondition(warrantNo, conditionOrderType, UFC::rwNotLock);
                stockDataPtr->UnLockForWrite();
    ...
    FLogPtr->fprintf(" %s.", logHead.c_str());
    FLogPtr->Flush();
}  //CEntrustRiskManagerClass::ProcessRiskRejectMessage()
//---------------------------------------------------------------------------------------------------------------------
void CEntrustRiskManagerClass::ProcessRiskBuySellMessage(TWSE::MessageHeader*& ReplyMsg, OrderRecord& OI)
{
    ...
    UFC::AnsiString logHead = "CEntrustRiskManagerClass::ProcessRiskBuySellMessage()";
            ...
            if (subAccountPtr != 0)
            {
                ...
                else if (buySellCode == 'S')
                {
                    ...
                    if (origSellOrderAmount >= differentAmount)
                        ...
                        FLogPtr->fprintf(" %s OrigSellOrderAmount[%15.4lf] < DifferentAmount[%15.4lf], Set SellOrderAmount to 0.0.", logHead.c_str(), origSellOrderAmount, differentAmount);
                    subAccountPtr->SetBuyOrderAmount(newSellOrderAmount, UFC::rwNotLock);
                    FLogPtr->fprintf(" %s Set Account[%s][%s] SellOrderAmount from [%15.4lf] to [%15.4lf].", logHead.c_str(), accountNo.c_str(), warrantNo.c_str(), origSellOrderAmount, newSellOrderAmount);
                }  //if (buySellCode == 'B')
                FLogPtr->fprintf(" %s %s.", logHead.c_str(), subAccountPtr->ToString(UFC::rwNotLock).c_str());
                subAccountPtr->UnLockForWrite();
            }  //if (subAccountPtr != 0)
    ...
    FLogPtr->fprintf(" %s", logHead.c_str());
    FLogPtr->Flush();
}  //CEntrustRiskManagerClass::ProcessRiskBuySellMessage()
//---------------------------------------------------------------------------------------------------------------------
void CEntrustRiskManagerClass::ProcessRiskCancelReplaceMessage(TWSE::MessageHeader*& ReplyMsg, OrderRecord& OI)
{
    int functionCode = OI.FuncCode;
    UFC::AnsiString logHead = "CEntrustRiskManagerClass::ProcessRiskCancelReplaceMessage()";
    TWSE::TMessageType msgType = ReplyMsg->MessageType();
    nsOrderMessageDefine::TradingSessionIDEnum tradingSessionID = nsOrderMessageDefine::tsAuto;
    int msgStatusCode = ReplyMsg->GetStatusCode();
    UFC::AnsiString accountNo, stockNo, orderID, orderQtyStr, beforeQtyStr, afterQtyStr, orderPriceStr;
    ...
    FLogPtr->fprintf(" %s", logHead.c_str());
    FLogPtr->Flush();
}  //CEntrustRiskManagerClass::ProcessRiskCancelReplaceMessage()
//----------------------------------------------------------------------------------------------------------------------
bool CEntrustRiskManagerClass::IsFillRptExecExist(const UFC::AnsiString& ExecKey, UFC::RWLockTypeEnum LockType)
{
    bool isExist = false;
    ...
    return isExist;
}  //CEntrustRiskManagerClass::IsFillRptExecExist()
//----------------------------------------------------------------------------------------------------------------------
void CEntrustRiskManagerClass::AddFillRptExec(const UFC::AnsiString& ExecKey, UFC::RWLockTypeEnum LockType)
{
    if (ExecKey.Length() <= 0) return;
    FillRptExecLock.Lock(LockType);
    FillRptExecKey.Add(ExecKey);
    FillRptExecLock.Unlock(LockType);
}  //CEntrustRiskManagerClass::AddFillRptExec()
//----------------------------------------------------------------------------------------------------------------------
void CEntrustRiskManagerClass::AddRiskMatchQty(TWSE::R3Body *R3MsgBodyPtr, const UFC::AnsiString& ExecKey)
{
    UFC::AnsiString logHead = "CEntrustRiskManagerClass::AddRiskMatchQty()";
        ...
        if (origLeavesQty > 0)
        {
            if (origLeavesQty >= matchQty)
            {
                ...
                if (accountPtr != 0)
                {
                    ...
                    CRMEntrustStockTradingData *stockDataPtr = accountPtr->FindStockTradingData(stockNo, UFC::rwLockForRead);
    ...
    FLogPtr->fprintf(" %s", logHead.c_str());
    FLogPtr->Flush();
}  //CEntrustRiskManagerClass::AddRiskMatchQty()
//------------------------------------------------------------------------------
void CEntrustRiskManagerClass::Execute(void)
{
    UFC::AnsiString logHead = "CEntrustRiskManagerClass::Execute()";
    int loopCount = 0;
...
//    CRiskManagerDataPool::DumpStockData();
    ...
    int checkFileInterval = gEntrustRMDataPoolPtr->GetCheckFileInterval();
    if (gEntrustRMDataPoolPtr->LoadRiskManagerControlData(false))
    {
        gEntrustRMDataPoolPtr->IncreaseRMCurrentControlFileSeqNo();
        while (gEntrustRMDataPoolPtr->LoadRiskManagerControlData(true))
            gEntrustRMDataPoolPtr->IncreaseRMCurrentControlFileSeqNo();
    }
    gEntrustRMDataPoolPtr->DumpAccountNoIndex(true, UFC::rwLockForRead);  //DumpDetail
    
    if (FIsRecoverMode) RecoverRMData();
    
    UFC::AnsiString stockWhiteListFilePathName = CEntrustRiskManagerDataPool::GetRMStockWhiteListFilePathName();
    if ((stockWhiteListFilePathName.Length() > 0) && (UFC::FileExists(stockWhiteListFilePathName) == TRUE))
    {
        ...
        {
            gEntrustRMDataPoolPtr->LoadStockWhiteList();
//            gEntrustRMDataPoolPtr->DumpAccountStockWhiteList(UFC::rwLockForRead);
            gEntrustRMDataPoolPtr->DumpRMStockWhiteListSet();
            FLogPtr->fprintf(" CEntrustRiskManagerClass::%s() WhiteList Time[%lld]", __func__, FWhiteListFileSecond);
            FLogPtr->fprintf(" CEntrustRiskManagerClass::%s()", __func__);
        }
    }

    while (gEntrustRMDataPoolPtr->LoadOrderDeleteData()) gEntrustRMDataPoolPtr->IncreaseRMCurrentDeleteFileSeqNo();    
    
    while (!IsTerminated())
    {
        loopCount++;
        FLogPtr->fprintf(" CEntrustRiskManagerClass::%s() Loop %d", __func__, loopCount);
        
        bool isLoadControlFileSuccess = false;
        if (gEntrustRMDataPoolPtr->GetRMCurrentControlFileSeqNo() <= 0)
            isLoadControlFileSuccess = gEntrustRMDataPoolPtr->LoadRiskManagerControlData(false);
        else
            isLoadControlFileSuccess = gEntrustRMDataPoolPtr->LoadRiskManagerControlData(true);
        ...
        if ((stockWhiteListFilePathName.Length() > 0) && (UFC::FileExists(stockWhiteListFilePathName) == TRUE))
        {
            ...
            if (UFC::GetFileModificationSecond(stockWhiteListFilePathName, newWhiteListFileSecond))
            {
                ...
                {
                    if (gEntrustRMDataPoolPtr->LoadStockWhiteList())
                    {
        ...
        if (gEntrustRMDataPoolPtr->LoadOrderDeleteData())
            gEntrustRMDataPoolPtr->IncreaseRMCurrentDeleteFileSeqNo();

        FLogPtr->Flush();
        UFC::SleepMS(checkFileInterval);
    }  //while (!IsTerminated())
}  //CEntrustRiskManagerClass::Execute()
}  //namespace twserm
