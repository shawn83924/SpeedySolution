...
void OrcTSEMapper::onStockExecution( ExecutionParser* Execution )
{
      ...
      if( (OrgOrd = FStore->GetOrder( mkStock, Execution->GetOrderID())) == NULL  ) ///< Use OrderID to query order object.
      {
...
{
    UFC::BufferedLog::DebugPrintf(" OnStockFilled");
    ...
    report.set( FIX::LastPx( (double)Execution->GetPrice()/100.00) );
