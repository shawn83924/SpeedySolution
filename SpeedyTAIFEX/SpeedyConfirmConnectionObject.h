...
//-----------------------------------------------------------------------------------------------------------------------
//   FCM Side: Connection Object for AP code 8 Target 3,8
//-----------------------------------------------------------------------------------------------------------------------
const long OPT_EXEC_QUEUE = 0x63A10000;
const long FUT_EXEC_QUEUE = 0x62A10000;
//----------------------------------------------------------------------------------------------------------------------
class SpeedyTMPCMConnectionObject : public ConnectionObjectBase, public UFC::PThread
{
private:
    SpeedyObjectsManager* FObjectManager;
	BOOL                 FNeedLock;
    ...
    UFC::AnsiString      FSID;
    UFC::PQueue          FExecQueue;
    UFC::UQueueData      FExecution;
private:
	// Implement interface PThread
	virtual void Execute( void );
private:
	void MessageLoop( void );
    void DisableLoop( void );	
	void ReceiveC030( TMP::TMPMessage* Execution );
	void ConfirmsubSystemHandshake( BOOL ReceivedL010 );
    void ReceiveExecution( void );
public:
	SpeedyTMPCMConnectionObject( SpeedyObjectsManager* ObjManager,
                                 ConnectionParameter&  info,
	  	                         ConnectionListener*   Listener,
		                         UFC::ThreadListener*  TListener,
		                         char		           APCode,
                                 BOOL                  IsOpt );
	virtual ~SpeedyTMPCMConnectionObject();
public:
    virtual void Listen( BOOL );
    virtual void Enable( BOOL );
    virtual void Active( void )  { UFC::PThread::Start(); }
