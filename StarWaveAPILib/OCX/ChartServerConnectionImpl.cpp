...
void TChartServerConnectionImpl::OnLastTick( const UFC::AnsiString& Exchange, const UFC::AnsiString& Symbol,TTickInfo& Tick )
{
	...
	TickInfoImpl->Copy(Tick);
	for( register size_t i=0;i< FEventList.size(); i ++ )
		FEventList[i]->OnLastTick( WideEx.c_bstr(), WideSym.c_bstr(),interface_cast<ITickInfo>(TickInfoImpl));
...
void TChartServerConnectionImpl::OnLastKBar( const UFC::AnsiString& Exchange, const UFC::AnsiString& Symbol,TMinuteKInfo& KBar )
{
	...
	MinuteKInfoImpl->Copy( KBar );
	for( register size_t i=0;i< FEventList.size(); i ++ )
		FEventList[i]->OnLastKBar( WideEx.c_bstr(), WideSym.c_bstr(),interface_cast<IMinuteKInfo>(MinuteKInfoImpl) );
...
{
	TTickInfoListImpl* TickInfoListImpl = new TTickInfoListImpl();
	WideString         WideEx( Exchange.c_str() );
	WideString         WideSym( Symbol.c_str() );

	TickInfoListImpl->Ref( &Ticks );
	for( register size_t i=0;i< FEventList.size(); i ++ )
		FEventList[i]->OnRecoverTicks( WideEx.c_bstr(), WideSym.c_bstr(),interface_cast<ITickInfoList>(TickInfoListImpl) );
	TickInfoListImpl->Release();
}
//------------------------------------------------------------------------------
...
{
	TMinuteKInfoListImpl* MinuteKInfoListImpl = new TMinuteKInfoListImpl();
	WideString            WideEx( Exchange.c_str() );
	WideString            WideSym( Symbol.c_str() );

	MinuteKInfoListImpl->Ref( &KBars );
	for( register size_t i=0;i< FEventList.size(); i ++ )
		FEventList[i]->OnRecoverKBars( WideEx.c_bstr(), WideSym.c_bstr(),interface_cast<IMinuteKInfoList>(MinuteKInfoListImpl) );
	MinuteKInfoListImpl->Release();
}
//------------------------------------------------------------------------------
							 ...
							 int Count, double MaxPx, double MinPx, UFC::PStringList& Date, UFC::List<TMinuteKInfoList*>& KBars )
{
	...
	for( int i =0; i < Count; i++ )
	{
		...
		MinuteKInfoListImpl->Ref( KBars[i] );
		for( register size_t i=0;i< FEventList.size(); i ++ )
			FEventList[i]->OnDayKBars( WideEx.c_bstr(), WideSym.c_bstr(), WideDate.c_bstr(), interface_cast<IMinuteKInfoList>(MinuteKInfoListImpl) );
		MinuteKInfoListImpl->Release();
	}
}
//------------------------------------------------------------------------------
