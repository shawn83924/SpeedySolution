...
:FInstance( Instance )
{
	FConnection = new MarketDataConnection( HInstance, AppName, ((IMarketDataConnectionEventListener*)this), TRUE, FALSE, FALSE, 32768 );
	UFC::SleepMS( 330 );
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
void MDConnection::OnMarketDataMessage( const UFC::AnsiString& Exchange, const UFC::AnsiString& Symbol, HKBrokerQueue* Msg )
{
...
// ---------------------------------------------------------------------------
void MDConnection::OnReload( const UFC::AnsiString& Exchange, UFC::PStringList& IDList, const UFC::AnsiString& ReloadID )
{
	if( FUIDSet.Exists( ReloadID ) == false )
	{
		FUIDSet.Add( ReloadID );
		new MDReloadThread( Exchange, IDList, FConnection );
	}
}
//------------------------------------------------------------------------------
void MDConnection::OnReloadComplete( const UFC::AnsiString& Exchange, UFC::PStringList& SymbolList )
{
	WideString ExStr( Exchange.c_str() );

	for( register size_t i=0;i< FInstance->FEventList.size(); i ++ )
		 FInstance->FEventList[i]->OnContractReloaded(ExStr.Copy());
}
// ---------------------------------------------------------------------------
