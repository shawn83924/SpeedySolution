...
#include "../COM/Lib/OrderConnection/TTaifexConnection.h"
#include "../COM/OrderConnection/Resource.h"
//----------------------------------------------------------------------------------------------------------------------
typedef enum
{
    crReject   = 0,
...
private:    
    ...
    double FDefOptCM;
    double GetFuturesDeposit( const UFC::AnsiString& SymbolPrefix );
    double GetOptionsCM( const UFC::AnsiString& SymbolPrefix );
    double GetOptionsA( const UFC::AnsiString& SymbolPrefix );
    void   UseMargin( const UFC::AnsiString& Account, UFC::AnsiString& AE, bool NeedLog = true );
private:
    ...
    void SendToAS400Checker( BOOL IsOpt, int NID, const UFC::AnsiString& Order, const UFC::AnsiString& Key );
    void LogExec( const char* Type ,TExecutionReportMessage* Msg, int Qty, double BeforeMargin, double ReleaseMargin, double CanUse );
private: 
...
public:
    ...
    double       GetCommodDeposit( const UFC::AnsiString& SymbolPrefix );
    ...
    void         InitMBus( MessageObject* MBusObj );
    void         SetResultListener( CheckResultListener* Listener ) { FResultListener = Listener; }      ;
    TCheckResult CheckOrder( CheckData& Obj );
};
//------------------------------------------------------------------------------
#endif
