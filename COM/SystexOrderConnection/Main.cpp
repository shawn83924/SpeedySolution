...
UFC::AnsiString GLogonID = "";
UFC::AnsiString FRecoverTimeFileName;
Int32           FOverlapTime = 60;
BOOL            GIsTAIFEX    = FALSE;
BOOL            GFillOnly    = FALSE;
BOOL            GConfirmOnly = FALSE;
...
public:
    ...
    {    
        ...
        {
            UFC::BufferedLog::Printf( " ***** Process SystexOrderConnection already exists *****"  );
            sleep( 2 );
            exit( 0 );
        }
    }
...
void GetRecoverBeginTime( void )
{
    ...
    ShmName.Printf( "SOC_%s_%s",GBrokerID.c_str(), GLogonID.c_str() );    
    ...
    {
        FRecoverBeginTime = TimeToInt( 0,0,0 );///< Reset time to 00:00:00.    
    }
...
void PrintHelp( void )
{
    printf( "SystexOrderConnection Copyright (c) September 2007 MDBS\n" );
    printf( "    -H                   Show this page.\n" );
    printf( "    -C [filename]        Config file name.(default=../cfg/SystexOrderConnection.cfg)\n" );
    printf( "    -B [BrokerID]        BrokerID.( Must have)\n" );
    printf( "    -N [LogonID]         LogonID.( default=BrokerID)\n" );
    printf( "    -TAIFEX              For TAIFEX. (Futures and Option)\n" );
    printf( "    -F                   First time start up.\n" );
    printf( "    -OC                  Send to backend Only Confirm execution.\n" );
    printf( "    -OF                  Send to backend Only Filled execution.\n" );
    exit( 1 );
}
//------------------------------------------------------------------------------
...
{
    FConfigFileName = "../cfg/SystexOrderConnection.cfg";
        ...
        for( int i = 1; i < argc; i++ )
        {
            ...
            {                 
                ...
                if( i < argc )
                {
                    ...
                    if( GLogonID.Length() == 0 )
                        throw( UFC::Exception("-L should before -R\n") );
                    FRecoverTimeFileName.Printf( "./SOC%s%sRecoverTime", GBrokerID.c_str() ,GLogonID.c_str() );
...
int main(int argc, char** argv)
{
    ...
    TSystexOrderConnection SystexOrderConnection( FConfigFileName, FRecoverBeginTime.ToInt32() );    
    ...
    while( TRUE )
    {
        ...
        UFC::SleepMS( 1000 );   
        if( SystexOrderConnection.GetConnectionState() == focReady )
            UpdateRecoverBeginTime();
        ///< Flush log file
        UFC::BufferedLog::FlushToFile();
        fflush( stdout );
        if( CheckConfigModifiaction() == TRUE )
            SystexOrderConnection.Recover( GetRecoverTime());
    }    
    return (EXIT_SUCCESS);
