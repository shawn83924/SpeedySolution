...
UFC::UTime      FTime;
UFC::AnsiString AppName = "SinoPacRMConnection";
UFC::AnsiString FRecoverTimeFileName = "SinoPacRM";
//------------------------------------------------------------------------------
...
void PrintHelp( void )
{    
    printf( "SinoPacRMConnection Copyright (c) September 2013 MDBS\n" );
...
int main(int argc, char** argv)
{
    ...
    SinoPacRMConnection SinoPacRM( Time );    
    SinoPacRM.Start();
    ...
    while( TRUE )
    {
        ...
        //UFC::BufferedLog::DebugPrintf( " Recover Begin Time:[%s]", GetRecoverTime().c_str() );
        UFC::BufferedLog::FlushToFile();
        fflush( stdout );
        if( Counter % 5 == 0 ) ///< Check connection every 5 sec         
            SinoPacRM.CheckConnection();
        if( CheckConfigModifiaction() == TRUE )
            SinoPacRM.Recover( GetRecoverTime());
        Counter++;
    }
