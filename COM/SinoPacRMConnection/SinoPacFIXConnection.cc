...
//------------------------------------------------------------------------------
void SinoPacExecConnection::OnLogonReply( const char* ReplyString, bool IsAccepted, int CID )
{
    if( IsAccepted )
    {
        FIsLogon = TRUE;
...
//------------------------------------------------------------------------------
void SinoPacExecConnection::ReduceOrder(  UFC::AnsiString& Order ) 
{
    ...
    if( Order[75] == '0' ) ///<0: Futures 1: Option
        ...
        Msg.SetMarket( nsOrderMessageDefine::mOptions );
...
void SinoPacExecConnection::onMsg(TibrvListener* listener, TibrvMsg& msg) 
{
    ...
    if( msgString != NULL )
    {
        ...
        switch( OrderBody[0] )
        {
            ...
            case '2': ReduceOrder( OrderBody );break;
...
//------------------------------------------------------------------------------
void SinoPacExecConnection::ConfirmToBackend( char Func, TExecutionReportMessage* Msg )
{
    ...
    UFC::AnsiString ot( (Msg->GetOrderType() == nsOrderMessageDefine::otMarket) ? "MKT" : "LMT" );
    UFC::AnsiString tif;
    char            DTRADE;
    char            ORDKND;
    ...
    int Digi = FConnection->GetPricePrecision( Msg->GetMarket(), Symbol );
    Dummy.PadThis( 32, ' ' );   
    ...
    {
        case nsOrderMessageDefine::osNew:       CDI = 'A';break;
        case nsOrderMessageDefine::osRejected: 
        {
            switch ( Msg->GetCxlRejResponseTo() ) 
            {
                case nsOrderMessageDefine::crrNew:     CDI = 'A'; break;
                case nsOrderMessageDefine::crrReplace: CDI = 'C'; break;
                case nsOrderMessageDefine::crrCancel:  CDI = 'D'; break;
                default:  CDI = ' '; break;
            }
            break;
        }
        case nsOrderMessageDefine::osReplaced:  CDI = 'C';break;
        case nsOrderMessageDefine::osCanceled:  CDI = 'D';break;
        default:  CDI = ' ';break;
    } 
    RTime.PadThis( 8, '0' );
    if (CODE.ToInt() != 0) 
...
//------------------------------------------------------------------------------
void SinoPacExecConnection::OnExecutionReport( TExecutionReportMessage* Msg, bool PossDup )
{
    switch( Msg->GetOrderStatus() )
    {
