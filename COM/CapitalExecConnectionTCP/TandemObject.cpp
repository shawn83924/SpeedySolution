...
const char *DescribeExecRptMarket(nsOrderMessageDefine::MarketEnum execRptMarket)
{
    switch (execRptMarket)
    {
        ...
        case nsOrderMessageDefine::mPATSFutures: return "PATS Futures";    //4, ///< PATS          Futures
        case nsOrderMessageDefine::mPATSOptions: return "PATS Options";    //5, ///< PATS          Options
...
//-----------------------------------------------------------------------------
Int32 CTandemMessage::GetTandemDataLength()
{
    return FTandemDataLength;
}  //CTandemMessage::GetTandemDataLength()
...
void CTandemMessage::InitField()
{
    ...
    FSubAccountNo        = "       ";               //62.ODK-SUB-MOTHER-ACNO     X(7)        355
    FOdrPrice            = 0;                       //63.ODK-ODR-PRRICE          S9(05)V9(3) 362
    FClearMemberID       = "    ";                  //64.ODK-CLEAR-MEMBER-ID     X(04)       371
    FInputSeqNo          = "              ";        //65.ODK-INPUT-SEQNO         S9(13)      375
    FInputKey            = "              ";        //66.ODK-INPUT-KEY           S9(13)      389
    FAp2InKind           = ' ';                     //67.ODK-AP2-IN-KIND         X(01)       403
    FFiller              = "                ";      //68.FILLER                  X(16)       404
    FSourceIP            = "                    ";  //69.ODK-SRC-IP              X(20)       420
}  //CTandemMessage::InitField()

//------------------------------------------------------------------------------
char CTandemMessage::TranslateExecType(nsOrderMessageDefine::ExecTypeEnum rptExecType, 
...
void CTandemMessage::SetFieldsValue(TExecutionReportMessage *ExecRptPtr)
{
    ...
    int clearMemberIndex = TMP::TMPMessage::GetCMID(ExecRptPtr->GetCMID());
...
//    FAEID = ExecRptPtr->GetAE();  //58.ODK-AE-ID 9(4) 346
...
//    UFC::AnsiString linBrnStr = ExecRptPtr->GetLINBRN();
//UFC::BufferedLog::Printf(" LINBRN[%s]", linBrnStr.c_str());
//UFC::BufferedLog::FlushToFile();
//    if (linBrnStr.Length() > 3)
//        FIBNO = linBrnStr.SubString(linBrnStr.Length() - 3, 3);
//    else
//    {
//        FIBNO = linBrnStr;
//        FIBNO.PadThis(3, ' ');
//    }
    ...
    if(FExecRptOrderStatus == nsOrderMessageDefine::osQuoteAccept)
        FOdrPrice = UFC::DoubleToInt(ExecRptPtr->GetPrice(), 3);
    else
        FOdrPrice = UFC::DoubleToInt(ExecRptPtr->GetLastPx(), 3);
    ...
    FFiller = "                ";              //68.FILLER          X(16)  404
    FSourceIP = userDataPtr->GetSourceIP();    //69.ODK-SRC-IP      X(20)  420
...
void CTandemMessage::GenerateTandemData()
{
    ...
    *CurPosition = FDosInKind;  //61.ODK-DOS-IN-KIND X(01) 354
    ...
    CurPosition += 7;
    tempStr.Printf("%+09d", FOdrPrice);  
    std::memcpy(CurPosition, tempStr.c_str(), 9);  //63.ODK-ODR-PRRICE S9(05)V9(3) 362
    ...
    *CurPosition = FAp2InKind;  //67.ODK-AP2-IN-KIND X(01) 403
    ...
    CurPosition += 1;
    std::memcpy(CurPosition, FFiller.c_str(), 16);  //68.FILLER X(16) 404
    
    CurPosition += 16;
    std::memcpy(CurPosition, FSourceIP.c_str(), 20);  //69.ODK-SRC-IP X(20) 420
