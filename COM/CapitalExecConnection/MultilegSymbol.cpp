...
//------------------------------------------------------------------------------------------------------------------------
UFC::UiniFile BO_SymbolDeCompose::FStrikePriceDigi( "../cfg/OptStrikePriceDigi.cfg" );
//------------------------------------------------------------------------------------------------------------------------
...
BOOL BO_SymbolDeCompose::GetExcercisePrice( const char* SymbolPos, const char* SymbolPricePos, char* ePrice )
{
    ...
    /*
    if ( !memcmp(SymbolPos, "TXO", 3) || !memcmp(SymbolPos, "TFO", 3) )
    {
        memcpy( ePrice, "0", 1 );
    ...
    UFC::AnsiString Prefix( SymbolPos, 3 );
    if( FStrikePriceDigi.GetValue( "Digi", Prefix, Value ) )
    {
        int Digi = Value.ToInt();
        UFC::BufferedLog::DebugPrintf( " [%s]=>Digi[%d]", SymbolPos, Digi );
        ...
        switch( Digi )
        {
            ...
            case 2:
                memcpy( ePrice, "000", 3 );
                memcpy( ePrice + 3, SymbolPricePos, 5 );
                strncpy_zero( ePrice + 8, "0", 1 );
                break;
            default: // default for digi 1
                memcpy( ePrice, "00", 2 );
                memcpy( ePrice + 2, SymbolPricePos, 5 );
                strncpy_zero( ePrice + 7, "00", 2 );
                break;
