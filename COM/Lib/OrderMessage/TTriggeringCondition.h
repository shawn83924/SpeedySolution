//#pragma once
#ifndef _TTRIGGERINGCONDITION_H_
#define _TTRIGGERINGCONDITION_H_
...
public :
    ...
    struct PrimitiveSignal // 基礎訊號
    {
        ...
        {
            pstRealTimePriceComp = 0, //  0:'p'    比較成交價(有成交才算)
            pstSellSidePriceComp,     //  1:'ssp'  比較內盤成交價
            pstBuySidePriceComp,      //  2:'bsp'  比較外盤成交價
            pstLastPriceComp,         //  3:'lp'   比較最後成交價
            pstBidPriceComp,          //  4:'bp'   比較買進價
            pstAskPriceComp,          //  5:'ap'   比較賣出價
            pstPeriodAccumVolumeComp, //  6:'pav' 比較單位時間內的累積量
            pstVolumeComp,            //  7:'v'    比較單量
            pstBidVolumeComp,         //  8:'bv'   比較委買量
            pstAskVolumeComp,         //  9:'av'   比較委賣量
            pstAccumVolumeComp,       // 10:'acv'  比較總量
            pstAnyTop5BidVolumeComp,  // 11:'a5bv' 比較五檔其中一檔委買量
            pstAnyTop5AskVolumeComp   // 12:'a5av' 比較五檔其中一檔委賣量
        } type;
        ...
        PrimitiveSignal(PrimitiveSignalType pst, LogicalComparisonOperator lco, unsigned int period_or_integer_part, unsigned int volume_or_decimal_part)
        {
            ...
            if (pst <= /*PrimitiveSignalType::*/pstAskPriceComp)
            {
                period = 0;
                volume_threshold = 0;
            ...
            else
            {
                ...
                if (pst == /*PrimitiveSignalType::*/pstPeriodAccumVolumeComp)
        ...
        std::string ToString(PrimitiveSignalType ps_type) const 
        {
            switch (ps_type)
            {
            case pstRealTimePriceComp:     return "p";
            case pstSellSidePriceComp:     return "ssp";
            case pstBuySidePriceComp:      return "bsp";
            case pstLastPriceComp:         return "lp";
            case pstBidPriceComp:          return "bp";
            case pstAskPriceComp:          return "ap";
            case pstPeriodAccumVolumeComp: return "pacv";
            case pstVolumeComp:            return "v";
            case pstBidVolumeComp:         return "bv";
            case pstAskVolumeComp:         return "av";
            case pstAccumVolumeComp:       return "acv";
            case pstAnyTop5BidVolumeComp:  return "a5bv";
            default: return "?";                
            }
        }
        ...
        {
            if (type <= /*PrimitiveSignalType::*/pstAskPriceComp)
    ...
    struct ConjunctionGate // 邏輯運算輸出訊號(邏輯閘)
    {
        ...
        std::vector<SignalID> input_from; // 輸入訊號來源
        std::vector<SignalID> output_to;  // 訊號輸出對象
    ...
    struct ExecutionTimer
    {
        ...
        ExecutionTimer() : enabled(false), hour(0), minute(0) {}
    };
...
private:
    ...
    bool StringToDecimal(const char* price, unsigned int decimal_locator, unsigned int& integer_part, unsigned int& decimal_part);
...
public:
    ...
    // 開始編輯觸價條件。之前設定的觸價條件(價量判斷或指定時間)皆會被清除
    void BeginEditing();
    ...
    // 設定觸價單執行時間
    // 注意：一般價量判斷條件跟時間條件只能擇一使用。有設定價量判斷的話時間條件就不會產生作用
    // return false:失敗 可能原因1:未呼叫 BeginEditing() 進入編輯模式 2：hour > 23 或 minute > 59
    //         true:成功
    bool SetExecutionTime(unsigned int hour, unsigned int minute);
