...
// Dual Interface : TRUE
// Event Support  : FALSE
// Description    : Message to place a new order
// ---------------------------------------------------------------------------
class DAX_COM_CLASS TNewOrderMessageImpl : public TCppAutoObject<INewOrderMessage>
{
  typedef _COM_CLASS inherited;
private:
	TNewOrderMessage FMessage;
public:
  __fastcall TNewOrderMessageImpl();
  __fastcall TNewOrderMessageImpl(const System::_di_IInterface Controller);
  __fastcall TNewOrderMessageImpl(Comobj::TComObjectFactory* Factory, const System::_di_IInterface Controller);
