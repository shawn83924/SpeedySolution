...
namespace Test1
{
    public partial class TOrderForm : Form    
    {
        ...
        public TOrderForm( )
        {
            ...
            String AppName = "OrderClient@" + System.Net.Dns.GetHostName();
            String MsgOut;

            FWhichAPI = SelectAPI();
            FOrderConnection.Create3( AppName, FWhichAPI, out MsgOut);            
            ...
            Text = "SpeedyOrderClient - Use API[ " + FWhichAPI.ToString() + " ]";
            toolStripStatusLabel1.Text = MsgOut;
            SetMarketButton();
            QueryPerformanceFrequency(out FFreq); 
        }
        //--------------------------------------------------------------------------------------------------------------------------------------------------
        SpeedyAPI.APIEnum SelectAPI()
        {
            SpeedyClient.SelAPIForm APIForm = new SpeedyClient.SelAPIForm();
            SpeedyAPI.APIEnum SelAPI = SpeedyAPI.APIEnum.aeSpeedy;                        

            APIForm.APIcomboBox.Items.Clear();
            if (FOrderConnection.SupportAPI(SpeedyAPI.APIEnum.aeSpeedy))
                APIForm.APIcomboBox.Items.Add(SpeedyAPI.APIEnum.aeSpeedy);
            if (FOrderConnection.SupportAPI(SpeedyAPI.APIEnum.aeCTP))
                APIForm.APIcomboBox.Items.Add(SpeedyAPI.APIEnum.aeCTP);
            if (FOrderConnection.SupportAPI(SpeedyAPI.APIEnum.aeFemas))
                APIForm.APIcomboBox.Items.Add(SpeedyAPI.APIEnum.aeFemas);
            if (FOrderConnection.SupportAPI(SpeedyAPI.APIEnum.aeHSctp))
                APIForm.APIcomboBox.Items.Add(SpeedyAPI.APIEnum.aeHSctp);
            if (FOrderConnection.SupportAPI(SpeedyAPI.APIEnum.aeHSufxFutOpt))
                APIForm.APIcomboBox.Items.Add(SpeedyAPI.APIEnum.aeHSufxFutOpt);
            if (FOrderConnection.SupportAPI(SpeedyAPI.APIEnum.aeHSufxStockOpt))
                APIForm.APIcomboBox.Items.Add(SpeedyAPI.APIEnum.aeHSufxStockOpt);
            if (FOrderConnection.SupportAPI(SpeedyAPI.APIEnum.aeKNctp))
                APIForm.APIcomboBox.Items.Add(SpeedyAPI.APIEnum.aeKNctp);
            if (FOrderConnection.SupportAPI(SpeedyAPI.APIEnum.aeKSFt))
                APIForm.APIcomboBox.Items.Add(SpeedyAPI.APIEnum.aeKSFt);
            if (FOrderConnection.SupportAPI(SpeedyAPI.APIEnum.aeKStrd))
                APIForm.APIcomboBox.Items.Add(SpeedyAPI.APIEnum.aeKStrd);           
            if( APIForm.APIcomboBox.Items.Count == 1 )
                return (SpeedyAPI.APIEnum)APIForm.APIcomboBox.Items[0];
            APIForm.APIcomboBox.SelectedIndex = 0;
            APIForm.ShowDialog();
            SelAPI = (SpeedyAPI.APIEnum)APIForm.APIcomboBox.Items[APIForm.APIcomboBox.SelectedIndex];
            return SelAPI;            
        }
        //--------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        void FOrderConnection_OnNews(SpeedyAPI.NewsMessage News)
        {
            ...
            if (News.Headline == "CTP")
            {
                ...
                {
                    SpeedyClient.SettlementInfoForm SettleForm = new SpeedyClient.SettlementInfoForm();

                    SettleForm.richTextBox.Text = News.Text;
                    if( SettleForm.ShowDialog() == DialogResult.OK )
                    {
                        SpeedyAPI.NewsMessage CTPReq = new SpeedyAPI.NewsMessage();
        ...
        {            
            try
            {
                if (PossDup == SpeedyAPI.ExecDupEnum.edNewExecution || PossDup == SpeedyAPI.ExecDupEnum.edSpeedyGenerate )
                {
                    FRecoverCount++;
        ...
        private void FNewOrder_Click(object sender, EventArgs e)
        {
            ...
            {
               SpeedyAPI.NewOrderMessage Msg = new SpeedyAPI.NewOrderMessage();

                Msg.AE = Program.FLogonID;
                Msg.Account = AccounttextBox.Text;
                Msg.Symbol = FSymbol.Text;
                Msg.OrderType = (SpeedyAPI.OrderTypeEnum)(FOrderType.SelectedItem);
                Msg.PositionEffect = (SpeedyAPI.PositionEffectEnum)(PEcomboBox.SelectedItem);
                Msg.Price = Program.ConvertStringToDouble(FPrice.Text);
                if( i == Count - 1 )
                    Msg.OrderQty = LastQty;
                else
                    Msg.OrderQty = SliceQty;
                Msg.Side = (SpeedyAPI.SideEnum)(FSide.SelectedItem);
                Msg.TradingSessionID = (SpeedyAPI.TradingSessionIDEnum)(SessionComboBox.SelectedItem);
                if ( FutButton.Checked == true )
                {
                    ...
                    {
                        Msg.Market =SpeedyAPI.MarketEnum.mCNFutures;
                        Msg.ExchangeCode = FExchange.Text;
                        Program.CNFUTSymbol = FSymbol.Text;
                        Program.CNFUTPx = FPrice.Text;
                    }
                    Msg.TimeInForce = (SpeedyAPI.TimeInForceEnum)(FTimeInForce.SelectedItem);
                    Msg.StrikePrice = 0;
                ...
                else if ( OptButton.Checked == true )
                {
                    ...
                    {
                        Msg.Market = SpeedyAPI.MarketEnum.mCNOptions;
                        Msg.ExchangeCode = FExchange.Text;
                ...
                {
                    Msg.Market = SpeedyAPI.MarketEnum.mTSE;
                ...
                {
                    Msg.Market = SpeedyAPI.MarketEnum.mPATSFutures;
                    Msg.ExchangeCode = FExchange.Text;
                ...
                {
                    Msg.Market = SpeedyAPI.MarketEnum.mOTC;
                    Msg.TSEStockSeqNo = Program.ConvertStringToInt32(StockSeqTextBox.Text); ;
                    Msg.Data = Program.UserData;
        ...
        //--------------------------------------------------------------------------------------------------------------------------------------------------
        void OnPartiallyFilled(string OrderID, Int64 NID, int MatchQty, SpeedyAPI.SideEnum Side, double Price, string Data)
        {
        ...
        //--------------------------------------------------------------------------------------------------------------------------------------------------
        private void NewOrder(SpeedyAPI.NewOrderMessage Msg )
        {
            C1.Win.C1FlexGrid.Row NewRow = FGrid.Rows.Add();
            FGrid.ShowCell( FGrid.Rows.Count -1, 0 );
            toolStripStatusLabel2.Text = Convert.ToSingle(FGrid.Rows.Count-1) +" Order records";
            ...
            Msg.NID = FOrderConnection.GenerateUniqueID(Msg.Market, SpeedyAPI.MessageTypeEnum.mtNew);
            NewRow["NID"] = Msg.NID;
            if (Msg.NID != 0)
        ...
        void OrderReplace(C1.Win.C1FlexGrid.Row ReplaceRow)
        {
            ...
            if ((OrderStatus == SpeedyAPI.OrderStatusEnum.osNew) || (OrderStatus == SpeedyAPI.OrderStatusEnum.osPartiallyFilled))
            {
                ...
                if (Market == SpeedyAPI.MarketEnum.mCNFutures || Market == SpeedyAPI.MarketEnum.mCNOptions)
                    return;                
        ...
        void OrderCancel(C1.Win.C1FlexGrid.Row CancelRow)
        {
            ...
            switch (Msg.Market)
            {
                ...
                case SpeedyAPI.MarketEnum.mOptions:
                    ...
                    Msg.Data = Program.UserData;
                    break;
                case SpeedyAPI.MarketEnum.mCNFutures:
                    Msg.Data = Program.UserData;
                    break;
                case SpeedyAPI.MarketEnum.mCNOptions:
                    Msg.StrikePrice = Convert.ToDouble(CancelRow["StrikePrice"]);
                    Msg.Data = Program.UserData;
                    break;
