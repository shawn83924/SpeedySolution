...
UFC::UTime      FTime;
BOOL            FFirstTimeStartup = FALSE;
UFC::AnsiString FConfigFileName( "../cfg/WLSExecConnection.cfg" );
//------------------------------------------------------------------------------
...
{
    UFC::AnsiString ShmName( "WSLExecConnection.RecoverBeginTime" );
...
void PrintHelp( void )
{
    printf( "WLSExecConnection Copyright (c) September 2007 MDBS\n" );
    printf( "    -F         First time start up.\n" );
    printf( "    -C         Config file. (Default:../cfg/WLSExecConnection.cfg).\n" );
...
{
    char Time[7];
    UFC::FileStreamEx File( "WLSTime", "w+" );	// "w+" = O_CREAT|O_TRUNC|O_RDWR
...
{
    UFC::FileStreamEx File( "WLSTime", "r+" );
    char Time[7];    
...
int main(int argc, char** argv)
{
    ...
    WLSExecConnection WLSExecConnection( FRecoverBeginTime.ToInt32() );
    CreateTimeFile( );
    ...
    while( TRUE )
    {
        ...
        if( CheckConfigModifiaction() == TRUE )
            WLSExecConnection.Recover( GetRecoverTime());
