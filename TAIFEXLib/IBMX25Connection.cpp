...
{
    Int32           PoolCount;
    long            PoolTime = TimeOut*1000 + us/1000;
    struct pollfd   PollFD[2];

    PollFD[0].fd     = FX25FD;
    PollFD[0].events = POLLWRNORM;
    PoolCount = poll( PollFD, 1, PoolTime );
    if( PoolCount > 0 )
        return TRUE;
    return FALSE;
}
...
void X25Connection::OpenStrem()
{
    if( FX25FD != -1 )
        close( FX25FD );
    FX25FD = open ("/dev/x25pkt", O_RDWR|O_NONBLOCK);// |O_NDELAY );
    if( FX25FD < 0 )
...
int X25Connection::CheckCTLCommand( struct strbuf* rec_ctl, BOOL& MoreData )
{
    ...
    if( rec_ctl->len != -1  ) ///< Received Control package
    {
        ...
        {
            if( (DataInd->DATA_xfer_flags & N_MORE_DATA_FLAG ) == 1 )            
