...
//---------------------------------------------------------------------------
TSpeedyFTPConnectionObject::TSpeedyFTPConnectionObject( ConnectionParameter& info, UFC::ThreadListener* TListener, ConnectionListener* CListener )
...
void TSpeedyFTPConnectionObject::Execute( void )
{
    ...
    X25Connection->SetLogManager( SpeedyObjectsManager::FileTransferLog );
    ...
    for( int i = 1; i <= PVC_BUSY_RETRY_TIMES; i++ )
    {
        ...
        {
            UFC::BufferedLog::Printf( " Socket Exception,Try to reconnect after 5 sec...." );
            X25Connection->Close();
            sleep( 5 );
        }
